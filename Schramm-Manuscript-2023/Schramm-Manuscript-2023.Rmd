...
abstract: |
  ...
  study uses Generalized Additive Models to assesses both watershed nutrient loading 
  and responses in estuary water quality to nutrient loading and streamflow.
  While delivered nutrient loads varied greatly, flow-normalized nutrient loads indicated
  limited evidence of changes in nutrient runoff in the watershed since 2005. 
  As a consequence, there wasn't strong evidence for an effect of nutrient loading
  (controlled for flow) on dissolved oxygen or chlorophyll-*a* concentrations.
  There was evidence total phosphorus (TP) loading had a strong positive
  effect on TP concentration in the Bay, particularly at sites closest to the
  river mouth. Seasonally-adjusted streamflow had strong effects on nutrient
  concentration and chlorophyll-*a* concentrations at most sites. Although
  temporal trends did not identify significant estuary water quality degredation 
  in dissolved oxygen or chlorophyll-*a* concentrations, site specific trends
  in both organic and inorganic nitrogen are concerning for the long-term
  health of the estuary. This study provides a baseline assessment of the
  linkages between watershed nutrient loading and water quality responses in 
  Lavaca Bay. Some of the data gaps highlighted in the study are being addressed
  in ongoing projects to provide improved information for resource managers.
# back matter
keywords: |
  estuary; nutrient loading; water quality; Texas
acknowledgement: |
...
funding: |
  This project was funded in part by a Texas Coastal Management Program grant approved by the Texas Land
  Commissioner, providing financial assistance under the Coastal Zone Management Act of 1972, as amended,
  awarded by the National Oceanic and Atmospheric Administration (NOAA), Office for Coastal Management,
  pursuant to NOAA Award No. NA21NOS4190136. The views expressed herein are those of the author(s) and do not
  necessarily reflect the views of NOAA, the U.S. Department of Commerce, or any of their subagencies
dataavailability: |
  Data and code are openly available in Zenodo at 
  https://doi.org/10.5281/zenodo.7330754.
conflictsofinterest: |
  The author declares no conflict of interest. The project sponsors had no 
...
hook_plot_wide <- function(x, options) {
  ...
  if(isTRUE(options$wideplot)) {
    x_out <- gsub(x = x_out,
                  ...
                  replacement='\\begin{figure}\\begin{adjustwidth}{-\\extralength}{0cm}',
                  fixed=TRUE)
...
Like many coastal areas globally, coastal watersheds along the Texas Gulf coast 
are facing pressures from increasing population, increases in point source 
and non-point source pollution and alterations to freshwater flows that affect
...
Christi, Texas. Lavaca Bay has faced substantial challenges associated with
...
stakeholders. @bugica_water_2020 identified monotonic increases in total 
phosphorus (TP), orthophosphate, total Kjeldahl nitrogen (TKN), and
chlorophyll-*a* at sites within Lavaca Bay. Although long-term changes in 
DO concentrations were not identified, the trends in nutrient concentrations are 
concerning due to the role of nitrogen as a limiting factor for primary 
production in many Texas estuaries [@gardnerNitrogenFixationDissimilatory2006; @houTransformationFateNitrate2012; @doradoUnderstandingInteractionsFreshwater2015; @paudelRelationshipSuspendedSolids2019; @wetz_exceptionally_2017] and the 
...
smooth function without explicit a priori knowledge of the expect shape, and (3)
the ability to specify a link function that relates the expected value of the
response to the linear predictors and avoid data transformations.
...
@kuhnert_quantifying_2012; @zhang_improving_2017], and 
expressed in @kuhnert_quantifying_2012 as:
...
quality response are in Table \ref{tab:estgammodels}.
...
from "satisfactory" to "very good" using median NSE, r^2^ and PBIAS 
calculated using repeated 5-fold cross validation on predicted and measured
...
```{r fig3, error=FALSE, message=FALSE, fig.height=4, fig.width=5, fig.cap="Aggregated estimated annual and flow-normalized annual NO\\textsubscript{3} and TP loads for USGS-08164000 and USGS-08164525.", out.width="100%"}
...
```{r fig4, error=FALSE, message=FALSE, fig.height=4, fig.width=5, fig.cap="Comparison of delivered annual loads at USGS-08164000 and USGS-08164525.", out.width="100%"}
load_lav_no3 <- tar_read(daily_no3_08164000, store = store)
load_nav_no3 <- tar_read(daily_no3_texana, store = store)
...
b <- ggplot(no3_annual) +
  ...
  labs(x = "", y = "Predicted Annual<br>NO<sub>3</sub> Load [kg]") +
  theme_TWRI_print() +
  theme(axis.title.y = element_markdown(size = 8),
...
```{r fig5, message=FALSE, warning=FALSE, fig.height=3.5, fig.width=6, fig.cap="Smoothed temporal trend component for water quality paramaters obtained from temporal estuary GAMs.", out.width="100%", wideplot=TRUE}
...
```{r fig6, message=FALSE, warning=FALSE, fig.height=3.5, fig.cap="Estimated effects of mean daily inflow residuals on mean TP, NO\\textsubscript{\\emph{x}}, chlorophyll-\\emph{a}, TKN, and DO concentrations in Lavaca Bay obtained from flow estuary GAMs.", out.width="100%", wideplot=TRUE}
...
in TP and NO~*x*~ concentration at all three sites (Figure \ref{fig:fig6}). At the upper-bay
...
```{r fig7, message=FALSE, warning=FALSE, fig.height=2.25, fig.width=5.5, fig.cap="Estimated effects of nutrient load residuals on TP and NO\\textsubscript{\\emph{x}} concentrations in Lavaca Bay obtained from flow+load estuary GAMs.", out.width="100%", wideplot=FALSE}
...
Other studies have published estimates of TP loading estimates for the Lavaca
River (USGS-08164000) over various time periods (Table \ref{tab:tpcomp}).
Converted to average annual yield, the estimates of TP loading in this study
are within the ranges of previous studies [@dunnTrendsNutrientInflows1996; @rebich_sources_2011; @omaniEstimationSedimentNutrient2014; @wise_spatially_2019], with the TP 
estimates in @dunnTrendsNutrientInflows1996 being notably lower. Given that none
...
relationships between nutrients and predictors. All of the water
quality data for the two river sites in the TCEQ database were ambient water
quality data, collected to be representative of typical conditions. This results
in few data collected at the highest portions of the flow duration curve
under which the majority of loadings take place. Supplementary flow-biased 
...
slightly from the trends identified by @bugica_water_2020. This is
...
estuaries that might be facing larger demands for freshwater diversions, higher
...
consistent with other studies relating freshwater inflow with nutrient 
...
Lavaca Bay. Tidal flushing from Matagorda Bay might be responsible for 
decreasing TKN concentrations and limiting the effects of freshwater inflow in 
lower reaches of Lavaca Bay. @russell_effect_2006 suggested the processing of 
organic loads in the upper portions of Lavaca Bay reduces the transport of 
nutrients into the lower reaches of the Bay.

Freshwater inflow also had a strong positive effect on chlorophyll-*a* at 
...
Navidad River. Given the relatively little variation in flow-normalized loads,
...
intertidal river limiting production [@russell_effect_2006].
...
decrease the prediction variance and probably improve accuracy at critical 
...
sources of nutrients and (2) that those changes have limited effect on 
water quality in Lavaca Bay to date. Although the study did not identify
strong responses to changes in nutrient loading, this does provide a baseline
assessment for future water quality management activities in the watershed.
